/**
 * @file wifi_rx_queue.c
 * @brief LinkG Wi-Fi接收等待队列实现
 * @author Dawn
 * @version 1.0.0
 * @date 2026-09-10
 */
 ...
 * @brief Wi-Fi接收等待队列。
 */
struct linkg_wifi_rx_queue
{
    linkg_wifi_rx_queue_item_t *items;    // 固定容量Queue元素数组
...
void linkg_wifi_rx_queue_destroy(linkg_wifi_rx_queue_t *queue)
{
    ...
    linkg_wifi_rx_queue_clear(queue);
...
{
    linkg_wifi_rx_queue_item_t *item;
    uint32_t                    head;
    uint32_t                    tail;
    uint32_t                    index;
...
uint32_t linkg_wifi_rx_queue_available(const linkg_wifi_rx_queue_t *queue)
{
    ...
    count = linkg_wifi_rx_queue_count(queue);

...
/**
 * @brief 将宽带模式芯片温度合并到统一Wi-Fi状态快照。
 */
static void _wifi_status_merge_temperature(linkg_wifi_status_snapshot_t *snapshot, int32_t temperature_c, bool temperature_valid)
{
    if (snapshot == NULL)
    {
        return;
    ...
    snapshot->local.chip_temperature_c     = temperature_c;
...
static int _wifi_status_refresh(void)
{
    ...
    ap_statistics_duplicated = runtime.role == LINKG_DEVICE_ROLE_AP && _wifi_status_ap_statistics_duplicated(&raw_status);
    ...
    query_temperature = runtime.work_mode == LINKG_WIFI_WORK_MODE_WIDE &&
                        interface_state != LINKG_WIFI_INTERFACE_STATE_DOWN;
    temperature_valid = false;

    if (query_temperature)
    {
        temperature_ret   = wifi_driver_get_temperature(&temperature_c);
        temperature_valid = temperature_ret == 0;

        if (temperature_ret != 0)
        {
            WIFI_STATUS_DEBUG("wide temperature unavailable, error=%d", temperature_ret);
        }
    }
    ...
    if (query_temperature)
    {
        _wifi_status_merge_temperature(&snapshot, temperature_c, temperature_valid);
    }

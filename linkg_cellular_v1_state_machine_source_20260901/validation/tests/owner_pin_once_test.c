...
static unsigned int g_running_checks;
static unsigned int g_pin_calls;
...
int cellular_status_process(uint64_t now_ms, cellular_status_refresh_mask_t requested)
{
    ...
    g_info.local.sim_state = LINKG_CELLULAR_SIM_STATE_PIN_REQUIRED;
...
int main(void)
{
    ...
    assert(ret == 0);
    assert(g_probe_count == 0U);
    assert(g_pin_calls == 1U);
    assert(!g_pdp_active);

...
static unsigned int g_usbnet_set_count;
static unsigned int g_nat_set_count;
static unsigned int g_mode_set_count;
static unsigned int g_sim_detect_set_count;
static unsigned int g_sim_urc_set_count;
static bool g_after_restart;
...
{
    (void)channel;
    *mode = g_after_restart ? RG255_USBNET_MODE_ECM : RG255_USBNET_MODE_RNDIS;
    return 0;
}
...
{
    (void)channel;
    *mode = g_after_restart ? RG255_NETWORK_CARD_MODE_NIC : RG255_NETWORK_CARD_MODE_ROUTER;
    return 0;
}
...
{
    (void)channel;
    *mode = g_after_restart ? LINKG_CELLULAR_NETWORK_MODE_AUTO : LINKG_CELLULAR_NETWORK_MODE_4G;
    return 0;
}
...
{
    (void)channel;
    config->enabled = g_after_restart;
    config->insert_level = g_after_restart
...
{
    (void)channel;
    status->enabled = g_after_restart;
...
int main(void)
{
    ...
    assert(g_restart_count == 1U);
    assert(g_usbnet_set_count == 1U);
    assert(g_nat_set_count == 1U);
    assert(g_mode_set_count == 1U);
    assert(g_sim_detect_set_count == 1U);
    assert(g_sim_urc_set_count == 1U);

...
static bool g_sim_removed;
static bool g_sim_event_emitted;
...
int cellular_monitor_take_events(cellular_monitor_events_t *events)
{
    ...
    if (g_probe_count >= 2U && !g_sim_event_emitted)
    {
        g_sim_event_emitted = true;
        g_sim_removed = true;
        events->mask = CELLULAR_MONITOR_EVENT_SIM_PRESENCE_CHANGED;
        events->sim_presence_valid = true;
...
int cellular_status_process(uint64_t now_ms, cellular_status_refresh_mask_t requested)
{
    ...
    g_info.local.sim_state = g_sim_removed
...
int main(void)
{
    ...
    assert(ret == 0);
    assert(g_probe_count == 2U);
    assert(g_sim_event_emitted);

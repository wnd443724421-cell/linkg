...
#define LQ_TUN_FRAGMENT_CLASS_CACHE_SIZE 64U
#define LQ_TUN_FRAGMENT_CLASS_TTL_MS     100U
...
struct tun_file {
	...
	struct ptr_ring tx_rings[LQ_TUN_TRAFFIC_CLASS_COUNT];
#if LQ_TUN_QSTAT
...
#define TUN_USER_FEATURES (NETIF_F_HW_CSUM|NETIF_F_TSO_ECN|NETIF_F_TSO| \
	...
	spinlock_t lq_traffic_lock;
	struct lq_tun_traffic_config lq_traffic_config;
	spinlock_t lq_fragment_lock;
	struct lq_tun_fragment_class_entry
		lq_fragment_classes[LQ_TUN_FRAGMENT_CLASS_CACHE_SIZE];
	u32 lq_fragment_next;
};
...
static void *lq_tun_ring_consume_class(struct tun_file *tfile,
				      u32 traffic_class)
{
	void *ptr;
...
static int lq_tun_resize_tx_rings(struct tun_file *tfile)
{
	...
	for (traffic_class = 0; traffic_class < LQ_TUN_TRAFFIC_CLASS_COUNT;
	     traffic_class++) {
		ret = ptr_ring_resize(&tfile->tx_rings[traffic_class],
				      lq_tun_class_queue_size(traffic_class),
...
static bool lq_tun_traffic_config_valid(const struct lq_tun_traffic_config *config)
{
	...
	if (!config || config->count > LQ_TUN_TRAFFIC_RULE_MAX)
		return false;

	for (index = 0; index < config->count; index++) {
		const struct lq_tun_traffic_rule *rule = &config->rules[index];

		if (rule->traffic_class >= LQ_TUN_TRAFFIC_CLASS_COUNT)
			return false;
		if (rule->protocol != LQ_TUN_PORT_PROTOCOL_TCP &&
		    rule->protocol != LQ_TUN_PORT_PROTOCOL_UDP)
			return false;
...
static bool lq_tun_fragment_key_equal(
	const struct lq_tun_fragment_class_entry *entry,
	__be32 source_addr, __be32 destination_addr,
	__be16 identification, u8 protocol)
{
	return entry->valid &&
...
static void lq_tun_fragment_class_store(struct tun_struct *tun,
					__be32 source_addr,
					__be32 destination_addr,
					__be16 identification,
					u8 protocol,
					u32 traffic_class)
{
	...
	if (!tun || traffic_class >= LQ_TUN_TRAFFIC_CLASS_COUNT)
		return;
	...
	for (index = 0; index < LQ_TUN_FRAGMENT_CLASS_CACHE_SIZE; index++) {
		...
		if (lq_tun_fragment_key_equal(entry, source_addr,
					     destination_addr,
					     identification,
					     protocol)) {
			candidate = entry;
		...
		if (!candidate && !entry->valid)
			candidate = entry;
	}

	if (!candidate) {
		candidate = &tun->lq_fragment_classes[
			tun->lq_fragment_next % LQ_TUN_FRAGMENT_CLASS_CACHE_SIZE];
...
static bool lq_tun_fragment_class_lookup(struct tun_struct *tun,
					 __be32 source_addr,
					 __be32 destination_addr,
					 __be16 identification,
					 u8 protocol,
...
static u32 lq_tun_classify_ports(struct tun_struct *tun, u8 protocol,
				 u16 source_port, u16 destination_port)
{
	...
	if (protocol == IPPROTO_TCP &&
	    (source_port == 22U || destination_port == 22U))
	...
	for (index = 0; index < config.count; index++) {
		...
		if ((source_port < rule->start_port ||
		     source_port > rule->end_port) &&
		    (destination_port < rule->start_port ||
		     destination_port > rule->end_port))
			continue;
...
static u32 lq_tun_classify_skb(struct tun_struct *tun, struct sk_buff *skb)
{
	...
	if (!tun || !skb || skb->protocol != htons(ETH_P_IP))
		return LQ_TUN_TRAFFIC_CLASS_DATA;

	network_offset = skb_network_offset(skb);
	ip_header = skb_header_pointer(skb, network_offset,
				       sizeof(ip_header_buffer),
				       &ip_header_buffer);
	if (!ip_header || ip_header->version != 4 || ip_header->ihl < 5)
		return LQ_TUN_TRAFFIC_CLASS_DATA;
	...
	/* 非首分片没有TCP/UDP端口，继承FIRST fragment已经记录的Class。 */
	if (fragment_offset != 0U) {
		if (lq_tun_fragment_class_lookup(tun,
						  ip_header->saddr,
						  ip_header->daddr,
						  ip_header->id,
						  ip_header->protocol,
	...
	ports = skb_header_pointer(skb, transport_offset, sizeof(ports_buffer),
				   ports_buffer);
	if (ports) {
		source_port = ntohs(ports[0]);
		destination_port = ntohs(ports[1]);
		traffic_class = lq_tun_classify_ports(tun,
						      ip_header->protocol,
						      source_port,
						      destination_port);
	}

	/* FIRST fragment缓存分类，后续fragment无需再次解析L4头。 */
	if (more_fragments)
		lq_tun_fragment_class_store(tun,
					     ip_header->saddr,
					     ip_header->daddr,
					     ip_header->id,
					     ip_header->protocol,
...
static long lq_tun_chr_set_traffic_config(struct file *file,
					  unsigned long arg)
{
	struct tun_file *tfile = file->private_data;
	struct lq_tun_traffic_config config;
...
static int lq_tun_batch_wait(struct tun_file *tfile, u32 traffic_class,
			     ktime_t deadline)
{
	ktime_t remaining;
	ktime_t now;
...
static long lq_tun_chr_read_batch(struct file *file, unsigned long arg)
{
	...
	while (read_pkts < req.max_pkts) {
		...
		/* 低优先级读取在消费下一包前先让位给已到达的高优先级队列。 */
		if (lq_tun_higher_priority_ready(tfile, req.traffic_class))
			break;

		if (read_pkts > 0 &&
		    ptr_ring_empty(&tfile->tx_rings[req.traffic_class])) {
			if (read_pkts >= req.min_pkts || req.timeout_us == 0 ||
			    lq_tun_higher_priority_ready(tfile, req.traffic_class))
				break;

			wait_ret = lq_tun_batch_wait(tfile, req.traffic_class,
						      deadline);
			if (wait_ret == -ETIME)
				break;
			if (wait_ret < 0)
				break;
			if (lq_tun_higher_priority_ready(tfile,
						       req.traffic_class))
				break;
		...
		ptr = tun_ring_recv_class(tfile, req.traffic_class,
					  read_pkts == 0 ? noblock : 1,
					  &ring_error);
		if (!ptr)
			n = ring_error;
		else
			 ...
			 unsigned int cmd, unsigned long arg)
{
	...
	if (cmd == LQ_TUN_IOC_SET_TRAFFIC_CONFIG)
		return lq_tun_chr_set_traffic_config(file,
					       (unsigned long)compat_ptr(arg));
...
static int tun_chr_open(struct inode *inode, struct file * file)
{
...

	{
		...
		for (traffic_class = 0;
		     traffic_class < LQ_TUN_TRAFFIC_CLASS_COUNT;
		     traffic_class++) {
			if (ptr_ring_init(&tfile->tx_rings[traffic_class],
					  lq_tun_class_queue_size(traffic_class),
					  GFP_KERNEL)) {

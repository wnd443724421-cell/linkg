 ...
 */
static int _wifi_link_open_socket(const struct in_addr *local_address,
                                  uint16_t local_port,
                                  int tos,
                                  ...
                                  int *out)
{
    ...
    *out = -1;
    ...
    LINKG_LOG_INFO("Wi-Fi UDP socket buffer, port=%u, sndbuf=%d, rcvbuf=%d",
                   local_port,
                   actual_send_buffer,
                   actual_receive_buffer);
 ...
 */
static int _wifi_link_send_batch(linkg_link_t *link,
                                 linkg_path_t *path,
                                 linkg_link_tx_class_t tx_class,
                                 const linkg_path_endpoint_t *destination,
                                 linkg_packet_t *const *packets,
                                 uint32_t count,
                                 int *results)
{
    linkg_wifi_link_t *wifi_link;
...
/**
 * @brief 获取指定对端的Wi-Fi链路累计接收和确认丢包统计。
 *
 * @note link必须为通过linkg_wifi_link_create创建的Wi-Fi具体Link。
 *       接口只返回Wi-Fi RX真实累计Counter，不计算时间窗口和Loss Rate。
 */
int linkg_wifi_link_get_rx_stats(linkg_link_t *link, const struct in_addr *peer_address, linkg_wifi_rx_stats_t *stats)
{
    linkg_wifi_link_t *wifi_link;
    ...
    if (link == NULL || peer_address == NULL || stats == NULL)
    {
        return -EINVAL;
    }
    ...
    ret = linkg_wifi_rx_get_peer_stats(wifi_link->rx,
                                       peer_address,
                                       &received_packets,
    ...
    stats->confirmed_lost_packets = confirmed_lost_packets;

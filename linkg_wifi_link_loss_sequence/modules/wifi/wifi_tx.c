...
#define LINKG_WIFI_TX_PEER_SLOT_COUNT       256U                                                                                 // 进程生命周期最多维护的Wi-Fi对端序列状态
#define LINKG_WIFI_TX_PEER_SLOT_MASK        (LINKG_WIFI_TX_PEER_SLOT_COUNT - 1U)                                                // 对端序列表掩码

_Static_assert((LINKG_WIFI_TX_PEER_SLOT_COUNT & (LINKG_WIFI_TX_PEER_SLOT_COUNT - 1U)) == 0U, "Wi-Fi TX peer slot count must be power of two");

/****************************** 内部类型 ******************************/

/**
 * @brief 单个Wi-Fi对端发送序列状态。
 *
 * 对端IPv4在TX对象生命周期内作为稳定Key。状态不随Path临时注销清除，避免同一进程内
 * Wi-Fi Path重建后Sequence回退导致接收端误判；TX对象销毁后整体重置。
 */
typedef struct
{
    struct in_addr address;       // 对端Wi-Fi IPv4
    uint32_t       next_sequence; // 下一个真正Socket提交时使用的Sequence
    bool           valid;         // 当前槽位是否有效
...
struct linkg_wifi_tx
{
    ...
    linkg_wifi_tx_peer_sequence_t peer_sequences[LINKG_WIFI_TX_PEER_SLOT_COUNT]; // 每个对端独立Wi-Fi Sequence
...
/**
 * @brief 计算对端IPv4对应的Sequence槽位起点。
 */
static uint32_t _linkg_wifi_tx_peer_hash(const struct in_addr *address)
{
    uint32_t value;
    ...
    return value & LINKG_WIFI_TX_PEER_SLOT_MASK;
}

/**
 * @brief 获取或创建指定对端的Wi-Fi TX Sequence状态。
 *
 * @note 调用方必须持有tx->lock。
 */
static linkg_wifi_tx_peer_sequence_t *_linkg_wifi_tx_peer_sequence_get_locked(linkg_wifi_tx_t *tx, const struct in_addr *address)
{
    linkg_wifi_tx_peer_sequence_t *state;
    uint32_t                       slot;
    uint32_t                       probe;

    if (tx == NULL || address == NULL || address->s_addr == htonl(INADDR_ANY))
    {
        return NULL;
    }

    slot = _linkg_wifi_tx_peer_hash(address);

    for (probe = 0U; probe < LINKG_WIFI_TX_PEER_SLOT_COUNT; probe++)
    {
        state = &tx->peer_sequences[slot];
        ...
        slot = (slot + 1U) & LINKG_WIFI_TX_PEER_SLOT_MASK;
    }
...
/**
 * @brief 计算当前Scratch中同一对端已临时预留的Sequence数量。
 */
static uint32_t _linkg_wifi_tx_peer_reserved_count(const linkg_wifi_tx_scratch_t *scratch, const linkg_wifi_tx_peer_sequence_t *state, uint32_t before_index)
{
    uint32_t count;
    uint32_t index;
    ...
    {
        if (scratch->peer_states[index] == state)
        {
            count++;
        }
    }
 ...
 *
 * 每个消息通过双iovec在Wire前置4B Wi-Fi Sequence Header，不修改原Packet内存。
 * Sequence先按当前对端next_sequence临时预留，仅对sendmmsg实际成功的连续前缀提交。
 *
 * @return 大于等于0表示成功提交的连续前缀数量，小于0表示sendmmsg错误。
 * @note 输入Queue元素、Socket和批次数量均由调用链保证有效；调用方必须持有tx->lock。
 */
static int _linkg_wifi_tx_send_once_locked(linkg_wifi_tx_t *tx, linkg_wifi_traffic_class_t traffic_class, linkg_wifi_tx_queue_item_t *items, uint32_t count)
{
    ...
    memset(scratch->peer_states, 0, count * sizeof(scratch->peer_states[0]));
    ...
    for (index = 0U; index < count; index++)
    {
        ...
        if (target->sin_family != AF_INET ||
            ...
            items[index].packet->data_length == 0U ||
            items[index].packet->data_length > LINKG_WIFI_TX_TRANSPORT_PAYLOAD_MAX)
        {
            return -EMSGSIZE;
        }

        peer_state = _linkg_wifi_tx_peer_sequence_get_locked(tx, &target->sin_addr);
        if (peer_state == NULL)
        {
            return -ENOSPC;
        }

        reserved_count = _linkg_wifi_tx_peer_reserved_count(scratch, peer_state, index);
        sequence       = peer_state->next_sequence + reserved_count;
    ...
    /**
     * sendmmsg成功数量表示数组前缀已经提交Socket。
     * 只对该连续前缀按各自Peer提交Sequence；未提交尾部下次会重新分配相同Sequence。
     */
 ...
 *
 * stopped为true表示本轮出现partial或发送压力，后续低优先级业务不得继续提交。
 * 调用方必须持有tx->lock。
 */
static int _linkg_wifi_tx_dispatch_class_locked(linkg_wifi_tx_t *tx, linkg_wifi_traffic_class_t traffic_class, uint32_t planned_count, bool *stopped)
{
    ...
    *stopped = false;
    ...
    while (remaining > 0U)
    {
        ...
        /**
         * partial表示本轮已经出现发送压力。
         * 未提交Packet继续保留在当前业务队列队首，后续低优先级业务本轮不得继续提交。
         */
        if (sent_count < send_count)
        {
            WIFI_DEBUG("TX sendmmsg部分发送，traffic=%u submit=%u sent=%u pending=%u",
 ...
 * 对端Sequence状态保留到TX对象销毁，保证同进程内Wi-Fi重启/Path重建不会回退序号。
 */
 ...
 *       对端Wi-Fi Sequence按IPv4在TX对象生命周期内保留，不因临时Path退役回退。
 */
 ...
 *       Wi-Fi Wire Sequence只在真正sendmmsg提交时分配，Queue中的Packet不占用Sequence。
 */

...
/**
 * @brief 完成单个TX帧的DMA描述符链配置。
 *
 * 设置首尾描述符标志，并根据set_ic决定是否在最后一个描述符上
 * 请求TX完成中断。启用硬件校验和时，为该帧使用的所有描述符
 * 配置校验和插入模式。
 */
void desc_tx_close(struct dma_desc *first, struct dma_desc *end, int csum_insert, bool set_ic)
{
	struct dma_desc *desc = first;

	first->desc1.tx.first_sg = 1;
	end->desc1.tx.last_seg = 1;
	end->desc1.tx.interrupt = set_ic;

...
_Static_assert(sizeof(struct lq_tun_batch_read) == 32U, "invalid TUN batch read ABI size");
...
/**
 * @brief 根据IPv4协议号转换为网络配置端口协议。
 */
static bool _linkg_tun_port_protocol_from_ipv4(uint8_t ipv4_protocol, linkg_network_port_protocol_t *protocol)
{
    if (protocol == NULL)
    {
        return false;
...
static bool _linkg_tun_traffic_rule_match(const linkg_tun_ipv4_info_t *info, linkg_tun_traffic_class_t *traffic_class)
{
    ...
    if (info == NULL || traffic_class == NULL || !info->ports_valid)
    {
        return false;
    }

    if (!_linkg_tun_port_protocol_from_ipv4(info->protocol, &protocol))
    {
        return false;
    }

    for (index = 0U; index < g_tun.traffic.count; index++)
    {
        rule = &g_tun.traffic.rules[index];
        ...
        if ((info->source_port < rule->start_port || info->source_port > rule->end_port) &&
            (info->destination_port < rule->start_port || info->destination_port > rule->end_port))
        {
            continue;
        }

        return _linkg_tun_traffic_class_from_network(rule->traffic_class, traffic_class);
    }
 ...
 */
static int _linkg_tun_read_batch(uint32_t packet_count, uint32_t *read_count)
{
...
int linkg_tun_init(linkg_packet_pool_t *packet_pool)
{
    ...
    LINKG_TUN_INFO("module initialized, batch=%u, min=%u, timeout=%uus, cpu=%d",

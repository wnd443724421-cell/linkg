...
typedef struct
{
    uint64_t received_bitmap[LINKG_TRANSPORT_RX_WINDOW_WORDS]; // 当前Peer当前Class滑动窗口位图，bit 0表示highest_sequence
    uint32_t highest_sequence;                                 // 当前Peer当前Class最高序列号
    uint32_t tracked_count;                                    // 当前窗口中已经进入协议观察范围的Sequence槽位数量，最大512
